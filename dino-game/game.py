...
def main():
    ...
    player = Dinosaur()
    # cloud = Cloud()
    game_speed = 20     #? variable que nos interesa para la red neuronal
    x_pos_bg = 0
    y_pos_bg = 380
    points = 0      #? variable que nos interesa para el algoritmo evolutivo
    font = pygame.font.Font('freesansbold.ttf', 20)
    obstacles = []      #? variable que nos interesa para la red neuronal
    death_count = 0
    ...
    while run:
        ...
        #! al agregar un nuevo obstaculo quizas ya podemos calcular la distancia al dinosaurio
        #! usar los obstaculos asi es mas sencillo, ya que vamos calculando agregando de a uno,
        #! calculamos la distancia y ahi empezamos a mandar todo a la red.
        if len(obstacles) == 0:
        ...
        for obstacle in obstacles:
            ...
            if isinstance(obstacle, Bird):
                bird_collision_rect = obstacle.rect.inflate(-80, -5)
                
                # Ajustar la posición vertical del dinosaurio para colisiones con aves
                dino_collision_rect.y = player.dino_rect.y  # Mantener la misma altura que el dinosaurio
            else:
                bird_collision_rect = obstacle.rect
